...
function CvExperience({ projects, onExplore, onOpenProject }) {
  ...
  return <div className="cv-experience">
    ...
    <main className="cv-main">
      ...
      <section id="cv-capacities" className="cv-section cv-capacities"><div className="cv-section-label">ROLES &amp; CAPACITIES</div><div><p className="section-lead">What I can do. Not titles — work I've finished.</p><div className="capacity-grid">{capacityGroups.map((group) => <article className="capacity-group" key={group.title}><h2>{group.title}</h2>{group.capacities.map((capacity) => <span key={capacity}>{capacity}</span>)}</article>)}</div></div></section>
